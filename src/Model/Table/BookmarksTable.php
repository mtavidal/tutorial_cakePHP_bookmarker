...
class BookmarksTable extends Table
{
    ...
    public function findTagged(array $options)
    {
        $bookmarks = $this->find()
...
protected function _buildTags($tagString)
    {
        $new = array_unique(array_map('trim', explode(',', strtolower($tagString))));
        $out = [];
        $query = $this->Tags->find()
            ->where(['LOWER(Tags.title) IN' => $new]);
        // Remove tags existentes da lista de novas tags.
        foreach ($query->extract('title') as $existing) {
            $index = array_search(strtolower($existing), $new);
            if ($index !== false) {
                unset($new[$index]);
            }
        }
